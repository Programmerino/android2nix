...
# (usage of it is discouraged for release, see android2nix/keystore.nix).
{
  ...
  latestBuildTools =
    foldl
    (acc: tools:
      if builtins.compareVersions acc.version tools.version == -1
      then tools
      else acc)
    {version = "";}
    androidComposition.build-tools;

  getBuildToolsBin = build-tools: "${build-tools}/libexec/android-sdk/build-tools/${build-tools.version}";
...
in
  stdenv.mkDerivation rec {
    ...
    installPhase = ''
      ...
      ${
        if extractApks
        then ''find . -name "*.apk" -exec cp {} $out/ \;''
        else "cp -r ./* $out/"
      }
    '';
